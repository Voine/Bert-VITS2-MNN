 ...
 */
class JPBV2Preprocess {

}
object JapaneseTextPreprocessor {
    ...
    fun text2sepKata(text: String): Triple<List<String>, List<String>, List<Pair<String, Int>>> {
        val parsed: List<OpenJTalkNode> = TODO("Call pyopenjtalk.run_frontend to get parsed nodes")
        ...
        for (node in parsed) {
            ...
            if (yomi.isNotEmpty()) {
                if (MARKS_REGEX.matches(yomi.substring(0, 1))) {
                    // 该部分读音是标点/符号
                    if (word.length > 1) {
                        // 多字符符号（如省略号）拆分成单个符号
                ...
                // pron 为空，表示无法获取读音（例如未知字符），保留原字符仅加入 sepText
                if (word.isNotEmpty()) {
                    sepText.add(word)
                }
            }
        }
        // 将收集的读音序列转换为片假名（保持与 sepText 对齐）
        val sepKata: List<String> = res.map { str ->
            TODO("Convert hiragana in str to katakana (jaconv.hira2kata)")
        }
        // 获取音素级别的重音信息
        val acc: List<Pair<String, Int>> = getAccent(parsed)
        return Triple(sepText, sepKata, acc)
    }
     ...
     */
    fun getAccent(parsed: List<OpenJTalkNode>): List<Pair<String, Int>> {
        val labels: List<String> = TODO("Call pyopenjtalk.make_label to get full context labels")
